...
runners.forEach(function(donor){
  fullNames.push(`${donor.first_name} ${donor.last_name}`);
});
...
firstNamesAllCaps = runners.map(function(names){
...
ticketPriceTotal = runners.reduce(function(total_donation , individual_donation){
 return total_donation + individual_donation.donation;
}, 0 )
...
// Problem 1 Anyone who donated more than 200$ gets a special medal for exceptional donations.

let greatDonation = [];

greatDonation = runners.filter(function(money){
  return money.donation >= 200;
})

console.log(greatDonation);

// Problem 2 Want to make a banner with all the companies who donated. Need an Array of just company names.
...
// Problem 3 The race director decided he likes big numbers better than small numbers (Still drunk with power) so he wanted all ids multiplied by 10

let multipliedByTen = [];

multipliedByTen = runners.map(function(ids){
  return (ids.id * 10);
});
